...
<div class="bg-white dark:bg-panel-dark border border-gray-200 dark:border-white/5 rounded-xl p-6 industrial-border">
    <form method="POST" action="<?php echo e(route('peminjam.borrowings.store')); ?>" id="borrowingForm">
        <?php echo csrf_field(); ?>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div>
                ...
                <input type="date" name="tanggal_pinjam" value="<?php echo e(old('tanggal_pinjam', date('Y-m-d'))); ?>" min="<?php echo e(date('Y-m-d')); ?>" class="w-full bg-gray-50 dark:bg-background-dark border border-gray-300 dark:border-gray-700 text-gray-900 dark:text-white text-sm rounded-lg focus:ring-primary focus:border-primary block p-2.5 transition-all" required>
            </div>
            <div>
                ...
                <input type="date" name="tanggal_selesai" value="<?php echo e(old('tanggal_selesai')); ?>" min="<?php echo e(date('Y-m-d')); ?>" class="w-full bg-gray-50 dark:bg-background-dark border border-gray-300 dark:border-gray-700 text-gray-900 dark:text-white text-sm rounded-lg focus:ring-primary focus:border-primary block p-2.5 transition-all" required>
                <p class="text-xs text-gray-500 mt-1 uppercase tracking-wide">Tanggal pengembalian yang diharapkan</p>
            </div>
        </div>

        <div class="mb-6">
            <label class="block text-xs font-bold text-gray-500 dark:text-gray-400 mb-3 uppercase tracking-widest pl-1">Pilih Alat</label>
            <div id="toolsContainer" class="space-y-4">
                <div class="tool-item flex gap-4 items-end border border-gray-200 dark:border-gray-700 p-4 rounded-lg bg-gray-50 dark:bg-gray-800/50">
                    <?php if(isset($selectedTool) && $selectedTool): ?>
                        <div class="flex-1">
                            <label class="block text-xs font-bold text-gray-500 dark:text-gray-400 mb-2 uppercase tracking-widest">Alat yang Dipilih</label>
                            <div class="flex items-center gap-3">
                                <div class="flex-1 p-3 bg-white dark:bg-gray-700/50 rounded-lg border border-gray-200 dark:border-gray-600">
                                    <div class="font-medium text-gray-900 dark:text-white"><?php echo e($selectedTool->nama_alat); ?></div>
                                    <div class="text-sm text-gray-500 dark:text-gray-400">Kategori: <?php echo e($selectedTool->category->nama_kategori); ?> | Stok: <?php echo e($selectedTool->stok_tersedia); ?></div>
                                </div>
                                <button type="button" onclick="changeTool(this)" class="px-4 py-2 bg-transparent border border-gray-400 dark:border-gray-600 text-gray-600 dark:text-gray-300 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 hover:text-gray-900 dark:hover:text-white transition-all font-medium text-sm">Ubah</button>
                            </div>
                            <input type="hidden" name="tools[0][tool_id]" value="<?php echo e($selectedTool->id); ?>" class="tool-id-input">
                        </div>
                    <?php else: ?>
                        <div class="flex-1">
                            ...
                            <select name="tools[0][tool_id]" class="w-full bg-white dark:bg-background-dark border border-gray-300 dark:border-gray-700 text-gray-900 dark:text-white text-sm rounded-lg focus:ring-primary focus:border-primary block p-2.5 transition-all tool-select" required>
                                ...
                                <?php $__currentLoopData = $tools; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $tool): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <option value="<?php echo e($tool->id); ?>" data-stok="<?php echo e($tool->stok_tersedia); ?>" <?php echo e(old('tools.0.tool_id') == $tool->id ? 'selected' : ''); ?>><?php echo e($tool->nama_alat); ?> (Stok: <?php echo e($tool->stok_tersedia); ?>)</option>
                    ...
                    <div class="w-32">
                        ...
                        <input type="number" name="tools[0][jumlah]" min="1" value="1" max="<?php echo e(isset($selectedTool) ? $selectedTool->stok_tersedia : ''); ?>" class="w-full bg-white dark:bg-background-dark border border-gray-300 dark:border-gray-700 text-gray-900 dark:text-white text-sm rounded-lg focus:ring-primary focus:border-primary block p-2.5 transition-all jumlah-input" required>
                    </div>
            ...
            <button type="button" onclick="addTool()" class="mt-4 h-[42px] px-5 bg-green-600 hover:bg-green-500 text-white rounded-lg font-medium transition-all flex items-center gap-2 shadow-lg shadow-green-600/20">
        ...
        <div class="flex gap-3 pt-6 border-t border-gray-200 dark:border-gray-700/50">
            <button type="submit" class="px-5 py-2.5 bg-primary hover:bg-primary/90 text-white rounded-lg transition-all flex items-center gap-2 text-sm font-medium shadow-lg shadow-primary/20">
...
const toolsData = [
    <?php $__currentLoopData = $tools; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $tool): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
    { id: <?php echo e($tool->id); ?>, nama: "<?php echo e($tool->nama_alat); ?>", stok: <?php echo e($tool->stok_tersedia); ?> },
    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
];
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\UKKfix\resources\views/peminjam/borrowings/create.blade.php ENDPATH**/ ?>