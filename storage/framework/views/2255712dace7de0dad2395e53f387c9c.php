...
<div class="max-w-2xl mx-auto">
    <h1 class="text-3xl font-bold text-white mb-6">Detail Peminjaman #<?php echo e($borrowing->id); ?></h1>
        ...
        <div class="space-y-4">
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <p class="font-medium text-white"><?php echo e($borrowing->tanggal_pinjam->format('d/m/Y')); ?></p>
                </div>
                <div>
                    <p class="text-sm text-gray-400">Tanggal Selesai</p>
                    <p class="font-medium text-white"><?php echo e($borrowing->tanggal_selesai ? $borrowing->tanggal_selesai->format('d/m/Y') : ($borrowing->jatuh_tempo ? $borrowing->jatuh_tempo->format('d/m/Y') : '-')); ?></p>
                </div>
            </div>
            <?php
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <span class="px-3 py-1 rounded-full text-sm font-semibold
                        ...
                        <?php elseif($borrowing->status == 'menunggu_pengembalian'): ?> bg-yellow-500/20 text-yellow-400
                ...
                <?php if($borrowing->status == 'disetujui'): ?>
                    <div>
                        <p class="text-sm text-gray-400">Estimasi Denda</p>
                        <?php if($estimatedFine['denda'] > 0): ?>
                            <p class="font-medium text-lg text-red-400">Rp <?php echo e(number_format($estimatedFine['denda'], 0, ',', '.')); ?></p>
                            <p class="text-xs text-gray-400"><?php echo e($estimatedFine['terlambat_hari']); ?> hari terlambat</p>
                        <?php else: ?>
                            <p class="font-medium text-green-400">Rp 0</p>
            ...
            <?php if($borrowing->status == 'disetujui' && isset($estimatedFine) && $estimatedFine['denda'] > 0): ?>
                <div class="bg-red-500/10 border border-red-500/30 rounded-lg p-4">
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            <p class="font-semibold text-red-400 mb-1">Peringatan Denda</p>
                            <p class="text-sm text-red-300">Peminjaman Anda telah melewati tanggal jatuh tempo. Segera kembalikan alat untuk menghentikan penambahan denda. Denda akan terus bertambah setiap harinya.</p>
                        </div>
                    </div>
                </div>
            <?php endif; ?>
            <div>
                <p class="text-sm text-gray-400 mb-3">Alat yang Dipinjam</p>
                <div class="space-y-3">
                    <?php $__currentLoopData = $borrowing->borrowingDetails; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $detail): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <div class="flex items-center gap-4 p-3 bg-gray-800/50 rounded-lg border border-gray-700">
                            ...
                            <div class="flex-shrink-0">
                                <?php if($detail->tool->gambar): ?>
                                    <img src="<?php echo e(asset($detail->tool->gambar)); ?>" 
                                         ...
                                         class="w-10 h-10 object-cover rounded-lg border-2 border-gray-700">
                                <?php else: ?>
                                    <div class="w-10 h-10 bg-gray-700 rounded-lg flex items-center justify-center border-2 border-gray-700">
                            ...
                            <div class="flex-1">
                                <h3 class="font-semibold text-white mb-1"><?php echo e($detail->tool->nama_alat); ?></h3>
                                <p class="text-sm text-gray-400"><?php echo e($detail->tool->category->nama_kategori); ?></p>
                            </div>
                            
                            
                            <div class="flex-shrink-0">
                                <div class="bg-blue-500/20 border border-blue-500/30 rounded-lg px-4 py-2 text-center">
                                    <p class="text-xs text-blue-400 mb-1">Jumlah</p>
                                    <p class="text-lg font-bold text-blue-300"><?php echo e($detail->jumlah); ?></p>
                                </div>
                            </div>
                        </div>
            ...
            <?php if($borrowing->keterangan): ?>
                <div>
                    ...
                    <p class="font-medium text-white"><?php echo e($borrowing->keterangan); ?></p>
                </div>
            <?php endif; ?>
            <?php if($borrowing->return): ?>
                <div class="border-t border-gray-700 pt-4 mt-4">
                    <p class="text-sm font-semibold text-gray-300 mb-2">Informasi Pengembalian</p>
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <p class="text-sm text-gray-400">Tanggal Kembali</p>
                            <p class="font-medium text-white"><?php echo e($borrowing->return->tanggal_kembali->format('d/m/Y')); ?></p>
                        </div>
                        <?php
                            $totalDenda = $borrowing->return->denda + ($borrowing->return->denda_kerusakan ?? 0);
                        ?>
                        <div>
                            <p class="text-sm text-gray-400">Total Denda Dibayar</p>
                            <p class="font-medium text-lg text-red-400">Rp <?php echo e(number_format($totalDenda, 0, ',', '.')); ?></p>
                            <?php if($borrowing->return->denda > 0): ?>
                                <p class="text-xs text-gray-400">Keterlambatan: Rp <?php echo e(number_format($borrowing->return->denda, 0, ',', '.')); ?>
                            ...
                            <?php if(($borrowing->return->denda_kerusakan ?? 0) > 0): ?>
                                <p class="text-xs text-gray-400">Kerusakan: Rp <?php echo e(number_format($borrowing->return->denda_kerusakan ?? 0, 0, ',', '.')); ?></p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
    ...
    <div class="mt-6">
        <?php if($borrowing->status == 'disetujui'): ?>
            <div class="mb-4 p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
                <p class="text-sm text-yellow-300">Klik tombol di bawah untuk mengajukan pengembalian. Petugas akan memeriksa kondisi alat dan menentukan denda jika ada kerusakan.</p>
            </div>
        <?php elseif($borrowing->status == 'menunggu_pengembalian'): ?>
            <div class="mb-4 p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
                <p class="text-sm text-yellow-300 font-semibold">Pengembalian Anda sedang menunggu persetujuan petugas. Petugas akan memeriksa kondisi alat dan menentukan denda jika ada kerusakan.</p>
            </div>
        <?php endif; ?>
        
        <div class="flex gap-3">
            <a href="<?php echo e(route('peminjam.borrowings.index')); ?>" class="px-5 py-2.5 bg-transparent border border-gray-600 text-gray-300 rounded-lg hover:bg-gray-700 hover:text-white transition-all font-medium text-sm flex items-center gap-2">
