...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="lg:col-span-1">
        <div class="bg-white dark:bg-panel-dark border border-gray-200 dark:border-white/5 rounded-xl p-6 industrial-border">
            @if($tool->gambar)
                <img src="{{ asset($tool->gambar) }}" alt="{{ $tool->nama_alat }}" class="w-full h-64 object-cover rounded-lg border border-gray-200 dark:border-gray-700">
            @else
        ...
        <div class="bg-white dark:bg-panel-dark border border-gray-200 dark:border-white/5 rounded-xl p-6 industrial-border">
            ...
            @if($tool->deskripsi)
                <div class="mb-6 p-4 bg-gray-50 dark:bg-gray-800/50 rounded-lg border border-gray-200 dark:border-gray-700/50">
                    ...
                    <p class="text-gray-600 dark:text-gray-300 leading-relaxed">{{ $tool->deskripsi }}</p>
                </div>
            @endif

            <div class="flex gap-3 pt-4 border-t border-gray-200 dark:border-gray-700/50">
                <a href="{{ route('peminjam.tools.index') }}" class="px-5 py-2.5 bg-transparent border border-gray-400 dark:border-gray-600 text-gray-600 dark:text-gray-300 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 hover:text-gray-900 dark:hover:text-white transition-all font-medium text-sm flex items-center gap-2">
                    <span class="material-symbols-outlined text-[18px]">arrow_back</span>
                    Kembali
                </a>
                @if($tool->stok_tersedia > 0 && $tool->status == 'tersedia')
                    <a href="{{ route('peminjam.borrowings.create', ['tool_id' => $tool->id]) }}" class="px-5 py-2.5 bg-primary hover:bg-primary/90 text-white rounded-lg transition-all flex items-center gap-2 text-sm font-medium shadow-lg shadow-primary/20">
