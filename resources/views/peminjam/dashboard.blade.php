...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
    ...
    <div class="bg-white dark:bg-panel-dark border border-gray-200 dark:border-white/5 rounded-xl p-6 industrial-border relative group overflow-hidden">
        ...
        <div class="relative z-10">
            ...
            <h3 class="text-3xl font-bold text-gray-900 dark:text-white mb-2">{{ $stats['peminjaman_aktif'] }}</h3>
    ...
    <div class="bg-white dark:bg-panel-dark border border-gray-200 dark:border-white/5 rounded-xl p-6 industrial-border relative group overflow-hidden">
        ...
        <div class="relative z-10">
            ...
            <h3 class="text-3xl font-bold text-gray-900 dark:text-white mb-2">{{ $stats['returned_borrowings'] }}</h3>
            <div class="h-1 w-full bg-gray-200 dark:bg-gray-800 rounded-full overflow-hidden">
    ...
    <div class="bg-white dark:bg-panel-dark border border-gray-200 dark:border-white/5 rounded-xl p-6 industrial-border relative group overflow-hidden">
        ...
        <div class="relative z-10">
            ...
            <h3 class="text-3xl font-bold text-gray-900 dark:text-white mb-2">{{ $stats['overdue_count'] }}</h3>
            <div class="h-1 w-full bg-gray-200 dark:bg-gray-800 rounded-full overflow-hidden">
        ...
        <div class="flex items-start gap-4">
            ...
            <div class="flex-1">
                ...
                <p class="text-gray-600 dark:text-gray-300 text-sm mb-2">
                    Ada <strong class="text-red-500 dark:text-red-400">{{ $stats['overdue_count'] }}</strong> peminjaman yang terlambat.<br>
                    <strong>Segera kembalikan!!</strong>
                </p>
                <a href="{{ route('peminjam.borrowings.index') }}" class="text-blue-500 dark:text-blue-400 hover:text-blue-400 dark:hover:text-blue-300 text-sm inline-flex items-center gap-1">
...
@elseif($nearest_due)
    ...
    <div class="card p-6 mb-6 border-l-4 {{ $isDueToday ? 'border-orange-500 bg-orange-500/5' : 'border-yellow-500 bg-yellow-500/5' }}">
        <div class="flex items-start gap-4">
            <div class="p-3 {{ $isDueToday ? 'bg-orange-500/10' : 'bg-yellow-500/10' }} rounded-full">
                <svg class="w-6 h-6 {{ $isDueToday ? 'text-orange-500' : 'text-yellow-500' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="flex-1">
                ...
                <p class="text-gray-600 dark:text-gray-300 text-sm">
                    Peminjaman #{{ $nearest_due->id }} - 
                    Jatuh tempo: <strong>{{ $nearest_due->jatuh_tempo->format('d/m/Y') }}</strong>
                    @if($isDueToday)
                        <span class="text-orange-500 dark:text-orange-400">(Hari ini)</span>
                    @else
                        <span class="text-yellow-600 dark:text-yellow-400">({{ abs(round($daysUntilDue)) }} hari lagi)</span>
                    @endif
                </p>
                <a href="{{ route('peminjam.borrowings.show', $nearest_due) }}" class="text-blue-400 hover:text-blue-300 text-sm mt-2 inline-flex items-center gap-1">
                    Lihat Detail →
                </a>
            </div>
        </div>
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
    ...
    <div class="bg-white dark:bg-panel-dark border border-gray-200 dark:border-white/5 rounded-xl industrial-border">
        <div class="p-6 border-b border-gray-200 dark:border-gray-700/50 flex justify-between items-center">
            ...
            <a href="{{ route('peminjam.tools.index') }}" class="text-sm text-primary hover:text-primary/80 font-bold uppercase tracking-wide inline-flex items-center gap-1 transition-colors">
                Lihat Semua
            ...
            <div class="grid grid-cols-1 gap-4">
                @forelse($available_tools as $tool)
                    <div class="p-4 bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700/50 rounded-lg hover:border-gray-300 dark:hover:border-gray-600 transition flex items-center gap-4">
                        <div class="flex-1">
                            <h3 class="font-semibold text-gray-900 dark:text-white">{{ $tool->nama_alat }}</h3>
                            <p class="text-sm text-gray-500 dark:text-gray-400">{{ $tool->category->nama_kategori }}</p>
                            <p class="text-sm text-gray-500 dark:text-gray-400">Stok: {{ $tool->stok_tersedia }}</p>
                            <div class="flex gap-2 mt-2">
                                @if($tool->status == 'tersedia' && $tool->stok_tersedia > 0)
                                    <a href="{{ route('peminjam.borrowings.create', ['tool_id' => $tool->id]) }}" class="px-3 py-1.5 bg-green-600 hover:bg-green-500 text-white rounded-lg text-xs font-medium inline-flex items-center gap-1 transition-all shadow-sm shadow-green-600/20">
                                        <span class="material-symbols-outlined text-[14px]">add</span>
                                        Pinjam
                                    </a>
                                @endif
                                <a href="{{ route('peminjam.tools.show', $tool) }}" class="px-3 py-1.5 bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 text-gray-700 dark:text-white rounded-lg text-xs font-medium inline-flex items-center gap-1 transition-all">
                        ...
                        @if($tool->gambar)
                            <div class="flex-shrink-0">
                                <img src="{{ asset($tool->gambar) }}" alt="{{ $tool->nama_alat }}" class="w-12 h-12 object-cover rounded-lg border border-gray-200 dark:border-gray-700">
                            </div>
                        @else
    ...
    <div class="bg-white dark:bg-panel-dark border border-gray-200 dark:border-white/5 rounded-xl industrial-border">
        <div class="p-6 border-b border-gray-200 dark:border-gray-700/50 flex justify-between items-center">
            ...
            <a href="{{ route('peminjam.borrowings.index') }}" class="text-sm text-primary hover:text-primary/80 font-bold uppercase tracking-wide inline-flex items-center gap-1 transition-colors">
                Lihat Semua
                <span class="material-symbols-outlined text-[16px]">arrow_forward</span>
            </a>
        </div>
                ...
                <div class="mb-4 p-4 bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700/50 rounded-lg hover:border-gray-300 dark:hover:border-gray-600 transition">
                    <div class="flex justify-between items-start mb-2">
                        <div>
                            ...
                            <p class="text-sm text-gray-500 dark:text-gray-400">{{ $borrowing->tanggal_pinjam->format('d/m/Y') }}</p>
                        </div>
                        <span class="px-2 py-1 text-xs font-bold rounded uppercase tracking-wider border
                            @if($borrowing->status == 'disetujui') bg-green-500/10 text-green-500 border-green-500/20
                            @elseif($borrowing->status == 'ditolak') bg-red-500/10 text-red-500 border-red-500/20
                            @elseif($borrowing->status == 'dikembalikan') bg-blue-500/10 text-blue-500 border-blue-500/20
                    ...
                    <a href="{{ route('peminjam.borrowings.show', $borrowing) }}" class="text-primary hover:text-primary/80 text-sm inline-flex items-center gap-1 font-bold uppercase tracking-wide transition-colors">
...
<div class="bg-white dark:bg-panel-dark border border-gray-200 dark:border-white/5 rounded-xl industrial-border p-6">
    <a href="{{ route('peminjam.borrowings.create') }}" class="btn btn-primary text-white inline-flex items-center gap-2">
