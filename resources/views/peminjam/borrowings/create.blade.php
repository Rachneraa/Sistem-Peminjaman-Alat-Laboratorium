    ...
    <form method="POST" action="{{ route('peminjam.borrowings.store') }}" id="borrowingForm">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div>
                ...
                <input type="date" name="tanggal_pinjam" value="{{ old('tanggal_pinjam', date('Y-m-d')) }}" min="{{ date('Y-m-d') }}" class="w-full bg-gray-50 dark:bg-background-dark border border-gray-300 dark:border-gray-700 text-gray-900 dark:text-white text-sm rounded-lg focus:ring-primary focus:border-primary block p-2.5 transition-all" required>
            </div>
            <div>
                ...
                <input type="date" name="tanggal_selesai" value="{{ old('tanggal_selesai') }}" min="{{ date('Y-m-d') }}" class="w-full bg-gray-50 dark:bg-background-dark border border-gray-300 dark:border-gray-700 text-gray-900 dark:text-white text-sm rounded-lg focus:ring-primary focus:border-primary block p-2.5 transition-all" required>
                        ...
                        <div class="flex-1">
                            ...
                            <div class="flex items-center gap-3">
                                <div class="flex-1 p-3 bg-white dark:bg-gray-700/50 rounded-lg border border-gray-200 dark:border-gray-600">
                                    <div class="font-medium text-gray-900 dark:text-white">{{ $selectedTool->nama_alat }}</div>
                                    <div class="text-sm text-gray-500 dark:text-gray-400">Kategori: {{ $selectedTool->category->nama_kategori }} | Stok: {{ $selectedTool->stok_tersedia }}</div>
                                </div>
                            ...
                            <input type="hidden" name="tools[0][tool_id]" value="{{ $selectedTool->id }}" class="tool-id-input">
                        </div>
                    @else
                        <div class="flex-1">
                            ...
                            <select name="tools[0][tool_id]" class="w-full bg-white dark:bg-background-dark border border-gray-300 dark:border-gray-700 text-gray-900 dark:text-white text-sm rounded-lg focus:ring-primary focus:border-primary block p-2.5 transition-all tool-select" required>
                                ...
                                @foreach($tools as $tool)
                                    <option value="{{ $tool->id }}" data-stok="{{ $tool->stok_tersedia }}" {{ old('tools.0.tool_id') == $tool->id ? 'selected' : '' }}>{{ $tool->nama_alat }} (Stok: {{ $tool->stok_tersedia }})</option>
                                @endforeach
                            </select>
                        </div>
                    @endif
