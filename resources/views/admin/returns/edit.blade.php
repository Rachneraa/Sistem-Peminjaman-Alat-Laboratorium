...
<div class="max-w-4xl mx-auto">
    <div class="mb-6">
        <h1 class="text-3xl font-bold text-gray-900 dark:text-white uppercase tracking-wider">Edit Pengembalian #{{ $return->id }}</h1>
        ...
        <div class="bg-white dark:bg-panel-dark border border-gray-200 dark:border-white/5 rounded-xl p-8 industrial-border">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <p class="text-gray-900 dark:text-white font-medium text-lg">{{ $return->borrowing->user->name }}</p>
                </div>
                <div>
                    <p class="text-[10px] text-gray-500 dark:text-gray-400 uppercase tracking-widest font-bold mb-1">ID Peminjaman</p>
                    <p class="text-gray-600 dark:text-gray-300 font-mono">#{{ $return->borrowing->id }}</p>
                </div>
            </div>
        </div>
            ...
            <div class="space-y-2">
                @foreach($return->borrowing->borrowingDetails as $detail)
                    <div class="flex justify-between items-center p-3 bg-gray-50 dark:bg-background-dark rounded-lg border border-gray-200 dark:border-gray-700/50">
                        <span class="text-sm text-gray-900 dark:text-gray-200 font-medium">{{ $detail->tool->nama_alat }}</span>
    ...
    <div class="bg-white dark:bg-panel-dark border border-gray-200 dark:border-white/5 rounded-xl p-8 industrial-border">
        <form method="POST" action="{{ route('admin.returns.update', $return) }}" id="returnForm">
            ...
            <div class="mb-6">
                ...
                <input type="number" name="denda" min="0" step="1000" value="{{ old('denda', $return->denda) }}" class="w-full bg-gray-50 dark:bg-background-dark border border-gray-300 dark:border-gray-700 text-gray-900 dark:text-white text-sm rounded-lg focus:ring-primary focus:border-primary block p-3 transition-all" placeholder="0">
            ...
            <div class="mb-6">
                ...
                <input type="number" name="terlambat_hari" min="0" value="{{ old('terlambat_hari', $return->terlambat_hari) }}" class="w-full bg-gray-50 dark:bg-background-dark border border-gray-300 dark:border-gray-700 text-gray-900 dark:text-white text-sm rounded-lg focus:ring-primary focus:border-primary block p-3 transition-all" placeholder="0">
            </div>
            ...
            <div class="mb-6">
                ...
                <input type="number" name="denda_kerusakan" min="0" step="1000" value="{{ old('denda_kerusakan', $return->denda_kerusakan ?? 0) }}" class="w-full bg-gray-50 dark:bg-background-dark border border-gray-300 dark:border-gray-700 text-gray-900 dark:text-white text-sm rounded-lg focus:ring-primary focus:border-primary block p-3 transition-all" placeholder="0">
            ...
            <div class="mb-8">
                ...
                <textarea name="keterangan" rows="4" class="w-full bg-gray-50 dark:bg-background-dark border border-gray-300 dark:border-gray-700 text-gray-900 dark:text-white text-sm rounded-lg focus:ring-primary focus:border-primary block p-3 transition-all" placeholder="Catatan kondisi alat, kerusakan yang ditemukan, dll...">{{ old('keterangan', $return->keterangan) }}</textarea>
            </div>
            ...
            <div class="flex space-x-3 pt-4 border-t border-gray-200 dark:border-gray-700/50">
                ...
                <a href="{{ route('admin.returns.index') }}" class="px-6 py-2.5 bg-transparent border border-gray-300 dark:border-gray-600 text-gray-600 dark:text-gray-300 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 hover:text-gray-900 dark:hover:text-white transition-all flex items-center gap-2 font-medium">
                    Batal
                </a>
            </div>
        </form>
