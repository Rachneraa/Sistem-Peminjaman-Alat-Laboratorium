...
@section('content')
    <div class="max-w-4xl mx-auto">
        <div class="mb-6">
            <h1 class="text-3xl font-bold text-gray-900 dark:text-white uppercase tracking-wider">Edit Pengembalian
                #{{ $return->id }}</h1>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            ...
            <div
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        <p class="text-[10px] text-gray-500 dark:text-gray-400 uppercase tracking-widest font-bold mb-1">ID
                            Peminjaman</p>
            ...
            <div
                ...
                <div class="space-y-2">
                    @foreach($return->borrowing->borrowingDetails as $detail)
                        <div
                            class="flex justify-between items-center p-3 bg-gray-50 dark:bg-background-dark rounded-lg border border-gray-200 dark:border-gray-700/50">
                            <span
                                class="text-sm text-gray-900 dark:text-gray-200 font-medium">{{ $detail->tool->nama_alat }}</span>
                            <span
                                class="text-xs font-bold text-primary px-2 py-1 bg-primary/10 rounded border border-primary/20">{{ $detail->jumlah }}
                                Unit</span>
                        </div>
                    @endforeach
                </div>
            ...
            <form method="POST" action="{{ route('admin.returns.update', $return) }}" id="returnForm">
                ...
                <div class="mb-6">
                    <label
                        ...
                        Kembali <span class="text-red-400">*</span></label>
                    <input type="date" name="tanggal_kembali"
                        value="{{ old('tanggal_kembali', $return->tanggal_kembali->format('Y-m-d')) }}"
                ...
                <div class="mb-6">
                    <label
                        class="block text-xs font-bold text-gray-500 dark:text-gray-400 mb-2 uppercase tracking-widest pl-1">Denda
                        Keterlambatan (Rp)</label>
                    <input type="number" name="denda" min="0" step="1000" value="{{ old('denda', $return->denda) }}"
                        class="w-full bg-gray-50 dark:bg-background-dark border border-gray-300 dark:border-gray-700 text-gray-900 dark:text-white text-sm rounded-lg focus:ring-primary focus:border-primary block p-3 transition-all"
                        placeholder="0">
                    <p class="text-[10px] text-gray-500 dark:text-gray-500 mt-2 uppercase tracking-wide">Denda untuk
                        keterlambatan pengembalian.</p>
                </div>

                <div class="mb-6">
                    <label
                        class="block text-xs font-bold text-gray-500 dark:text-gray-400 mb-2 uppercase tracking-widest pl-1">Hari
                        Terlambat</label>
                    <input type="number" name="terlambat_hari" min="0"
                        value="{{ old('terlambat_hari', $return->terlambat_hari) }}"
                        class="w-full bg-gray-50 dark:bg-background-dark border border-gray-300 dark:border-gray-700 text-gray-900 dark:text-white text-sm rounded-lg focus:ring-primary focus:border-primary block p-3 transition-all"
                        placeholder="0">
                </div>
                ...
                <div class="mb-6">
                    ...
                    <input type="number" name="denda_kerusakan" min="0" step="1000"
                        value="{{ old('denda_kerusakan', $return->denda_kerusakan ?? 0) }}"
                        class="w-full bg-gray-50 dark:bg-background-dark border border-gray-300 dark:border-gray-700 text-gray-900 dark:text-white text-sm rounded-lg focus:ring-primary focus:border-primary block p-3 transition-all"
                        placeholder="0">
                    <p class="text-[10px] text-gray-500 dark:text-gray-500 mt-2 uppercase tracking-wide">Denda untuk
                        kerusakan alat.</p>
                ...
                <div class="mb-8">
                    <label
                        class="block text-xs font-bold text-gray-500 dark:text-gray-400 mb-2 uppercase tracking-widest pl-1">Keterangan
                        / Catatan Kondisi Alat</label>
                    <textarea name="keterangan" rows="4"
                        class="w-full bg-gray-50 dark:bg-background-dark border border-gray-300 dark:border-gray-700 text-gray-900 dark:text-white text-sm rounded-lg focus:ring-primary focus:border-primary block p-3 transition-all"
                        placeholder="Catatan kondisi alat, kerusakan yang ditemukan, dll...">{{ old('keterangan', $return->keterangan) }}</textarea>
                </div>
