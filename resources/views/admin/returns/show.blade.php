...
<div class="max-w-4xl mx-auto">
    <div class="mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div class="flex flex-wrap gap-2">
            ...
            <a href="{{ route('admin.returns.edit', $return) }}" class="px-5 py-2.5 bg-yellow-600 hover:bg-yellow-500 text-white rounded-lg transition-all flex items-center gap-2 font-medium shadow-lg shadow-yellow-600/20">
                ...
                <div class="flex justify-between items-center p-3 bg-gray-50 dark:bg-background-dark rounded border border-gray-200 dark:border-gray-700/50">
                    <div>
                        ...
                        @if($return->terlambat_hari > 0)
                            <span class="text-[10px] text-red-500 dark:text-red-400 font-bold uppercase tracking-wider">{{ $return->terlambat_hari }} Hari</span>
                        @endif
                    </div>
                    <span class="text-gray-900 dark:text-white font-mono">Rp {{ number_format($return->denda, 0, ',', '.') }}</span>
                </div>
                
                <div class="flex justify-between items-center p-3 bg-gray-50 dark:bg-background-dark rounded border border-gray-200 dark:border-gray-700/50">
                    <p class="text-xs text-gray-600 dark:text-gray-400">Kerusakan</p>
                    <span class="text-gray-900 dark:text-white font-mono">Rp {{ number_format($return->denda_kerusakan ?? 0, 0, ',', '.') }}</span>
                </div>
                ...
                <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                    @foreach($return->borrowing->borrowingDetails as $detail)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition">
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white font-medium">{{ $detail->tool->nama_alat }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{{ $detail->tool->category->nama_kategori }}</td>
