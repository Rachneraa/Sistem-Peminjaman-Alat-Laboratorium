    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="bg-panel-dark border border-white/5 rounded-xl p-8 industrial-border">
            <form method="POST" action="{{ route('admin.returns.store') }}" id="returnForm">
                ...
                <div class="mb-6">
                    <label class="block text-xs font-bold text-gray-400 mb-2 uppercase tracking-widest pl-1">Peminjaman
                        <span class="text-red-400">*</span></label>
                    <select name="borrowing_id" id="borrowingSelect"
                        class="w-full bg-background-dark border border-gray-700 text-white text-sm rounded-lg focus:ring-primary focus:border-primary block p-3 transition-all"
                        required autofocus>
                    ...
                    <p class="text-[10px] text-gray-500 mt-2 uppercase tracking-wide">Hanya menampilkan peminjaman yang
                        disetujui atau menunggu pengembalian dan belum dikembalikan.</p>
                </div>
                ...
                <div id="borrowingDetails" class="mb-8 hidden animate-fade-in-up">
                    <div class="p-6 bg-background-dark rounded-xl border border-gray-700/50">
                        <p
                            class="text-xs font-bold text-gray-400 mb-4 uppercase tracking-widest border-b border-gray-700 pb-2">
                            Detail Peminjaman</p>
                ...
                <div class="mb-6">
                    <label class="block text-xs font-bold text-gray-400 mb-2 uppercase tracking-widest pl-1">Tanggal Kembali
                        <span class="text-red-400">*</span></label>
                ...
                <div class="mb-6">
                    <label class="block text-xs font-bold text-gray-400 mb-2 uppercase tracking-widest pl-1">Denda Kerusakan
                        (Rp)</label>
                    <input type="number" name="denda_kerusakan" min="0" step="1000" value="{{ old('denda_kerusakan', 0) }}"
                        class="w-full bg-background-dark border border-gray-700 text-white text-sm rounded-lg focus:ring-primary focus:border-primary block p-3 transition-all"
                        placeholder="0">
                    <p class="text-[10px] text-gray-500 mt-2 uppercase tracking-wide">Masukkan jumlah denda kerusakan jika
                        ada kerusakan pada alat yang dikembalikan.</p>
                </div>

                <div class="mb-8">
                    <label class="block text-xs font-bold text-gray-400 mb-2 uppercase tracking-widest pl-1">Keterangan /
                        Catatan Kondisi Alat</label>
                    <textarea name="keterangan" rows="4"
                        class="w-full bg-background-dark border border-gray-700 text-white text-sm rounded-lg focus:ring-primary focus:border-primary block p-3 transition-all"
                        placeholder="Catatan kondisi alat, kerusakan yang ditemukan, dll...">{{ old('keterangan') }}</textarea>
                </div>
                ...
                <div class="flex space-x-3 pt-4 border-t border-gray-700/50">
                    ...
                    <a href="{{ route('admin.returns.index') }}"
                        class="px-6 py-2.5 bg-transparent border border-gray-600 text-gray-300 rounded-lg hover:bg-gray-700 hover:text-white transition-all flex items-center gap-2 font-medium">
                        Batal
                    </a>
                </div>
            </form>
