    ...
    <div class="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
            ...
            <div class="flex items-center gap-3 mt-2">
                ...
                <span class="text-sm text-gray-500 dark:text-gray-400 font-mono">#{{ $borrowing->id }}</span>
            </div>
        </div>
        <div class="flex gap-2 w-full sm:w-auto">
            <a href="{{ route('admin.borrowings.index') }}" class="flex-1 sm:flex-initial px-4 sm:px-5 py-2.5 bg-transparent border border-gray-300 dark:border-gray-600 text-gray-600 dark:text-gray-300 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 hover:text-gray-900 dark:hover:text-white transition-all flex items-center justify-center gap-2 font-medium">
                <span class="material-symbols-outlined text-[20px]">arrow_back</span>
                Kembali
            </a>
            <a href="{{ route('admin.borrowings.edit', $borrowing) }}" class="flex-1 sm:flex-initial px-4 sm:px-5 py-2.5 bg-yellow-600 hover:bg-yellow-500 text-white rounded-lg transition-all flex items-center justify-center gap-2 font-medium shadow-lg shadow-yellow-600/20">
        ...
        <div class="bg-white dark:bg-panel-dark border border-gray-200 dark:border-white/5 rounded-xl p-5 sm:p-8 industrial-border">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <p class="text-gray-900 dark:text-white font-medium text-lg">{{ $borrowing->user->name }}</p>
                </div>
                <div>
                    <p class="text-[10px] text-gray-500 dark:text-gray-400 uppercase tracking-widest font-bold mb-1">Email</p>
                    <p class="text-gray-600 dark:text-gray-300 font-medium">{{ $borrowing->user->email }}</p>
                </div>
            </div>
        </div>
            ...
            <div class="space-y-4">
                <div class="flex justify-between items-start">
                    <div>
                        ...
                        <span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider
                            @if($borrowing->status == 'disetujui') bg-green-500/10 text-green-400 border border-green-500/20
                            @elseif($borrowing->status == 'ditolak') bg-red-500/10 text-red-400 border border-red-500/20
                            @elseif($borrowing->status == 'dikembalikan') bg-blue-500/10 text-blue-400 border border-blue-500/20
                    ...
                    <div class="text-right">
                        ...
                        <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ $borrowing->borrowingDetails->sum('jumlah') }}</p>
                    </div>
                </div>
                <div class="grid grid-cols-2 gap-4 pt-2">
                    <div class="bg-gray-50 dark:bg-background-dark p-3 rounded-lg border border-gray-200 dark:border-gray-700/50">
                        ...
                        <p class="text-gray-900 dark:text-white font-mono">{{ $borrowing->tanggal_pinjam->format('d/m/Y') }}</p>
                    </div>
                    <div class="bg-gray-50 dark:bg-background-dark p-3 rounded-lg border border-gray-200 dark:border-gray-700/50">
                        <p class="text-[10px] text-gray-500 dark:text-gray-400 uppercase tracking-widest font-bold mb-1">Selesai</p>
                        <p class="text-gray-900 dark:text-white font-mono">{{ $borrowing->tanggal_selesai ? $borrowing->tanggal_selesai->format('d/m/Y') : '-' }}</p>
                    </div>
                </div>
            </div>
        </div>
            ...
            @foreach($borrowing->borrowingDetails as $detail)
                <div class="flex items-start gap-4 p-4 bg-gray-50 dark:bg-background-dark rounded-xl border border-gray-200 dark:border-gray-700/50 hover:border-primary/30 transition-all group">
                    ...
                    <div class="flex-1 min-w-0">
                        <h4 class="font-bold text-gray-900 dark:text-white text-lg mb-1 truncate group-hover:text-primary transition-colors">{{ $detail->tool->nama_alat }}</h4>
                        <p class="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2">{{ $detail->tool->category->nama_kategori }}</p>
                    ...
                    <div class="flex-shrink-0 text-center px-4">
                        ...
                        <div class="text-2xl font-bold text-primary">{{ $detail->jumlah }}</div>
                    </div>
                </div>
            @endforeach
        </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="bg-gray-50 dark:bg-gray-800/50 p-5 sm:p-6 rounded-xl border border-gray-200 dark:border-gray-700/50">
                        ...
                        <div class="space-y-2 mb-4 border-b border-gray-200 dark:border-gray-700 pb-4">
                            @if($borrowing->return->denda > 0)
                                <div class="flex justify-between text-sm">
                                    <span class="text-gray-600 dark:text-gray-400">Keterlambatan @if($borrowing->return->terlambat_hari > 0) ({{ $borrowing->return->terlambat_hari }} hari) @endif</span>
                                    <span class="text-red-500 dark:text-red-400 font-mono">Rp {{ number_format($borrowing->return->denda, 0, ',', '.') }}</span>
                                </div>
                            @endif
                            @if(($borrowing->return->denda_kerusakan ?? 0) > 0)
                                <div class="flex justify-between text-sm">
                                    <span class="text-gray-600 dark:text-gray-400">Kerusakan</span>
                                    <span class="text-red-500 dark:text-red-400 font-mono">Rp {{ number_format($borrowing->return->denda_kerusakan ?? 0, 0, ',', '.') }}</span>
                                </div>
                            @endif
                            @if($totalDenda == 0)
                                <div class="flex justify-between text-sm">
                        ...
                        <div class="flex justify-between items-end">
                            ...
                            <span class="text-2xl font-bold text-red-500 font-mono">Rp {{ number_format($totalDenda, 0, ',', '.') }}</span>
                        </div>
                    </div>
                </div>
            </div>
