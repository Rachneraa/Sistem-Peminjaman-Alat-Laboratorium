...
@section('content')
    <div class="max-w-4xl mx-auto">
        <div class="mb-6">
            <h1 class="text-3xl font-bold text-gray-900 dark:text-white uppercase tracking-wider">Edit Peminjaman
                #{{ $borrowing->id }}</h1>
            ...
            <form method="POST" action="{{ route('admin.borrowings.update', $borrowing) }}" id="borrowingForm">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    ...
                    <div>
                        ...
                        <input type="date" name="tanggal_selesai"
                            value="{{ old('tanggal_selesai', $borrowing->tanggal_selesai ? $borrowing->tanggal_selesai->format('Y-m-d') : '') }}"
                            class="w-full bg-gray-50 dark:bg-background-dark border border-gray-300 dark:border-gray-700 text-gray-900 dark:text-white text-sm rounded-lg focus:ring-primary focus:border-primary block p-3 transition-all"
                            required>
                    </div>
                </div>
                ...
                <div class="mb-6">
                    ...
                    <select name="status"
                        class="w-full bg-gray-50 dark:bg-background-dark border border-gray-300 dark:border-gray-700 text-gray-900 dark:text-white text-sm rounded-lg focus:ring-primary focus:border-primary block p-3 transition-all"
                        required>
                        <option value="menunggu" {{ old('status', $borrowing->status) == 'menunggu' ? 'selected' : '' }}>
                            Menunggu</option>
                        <option value="disetujui" {{ old('status', $borrowing->status) == 'disetujui' ? 'selected' : '' }}>
                            Disetujui</option>
                        <option value="ditolak" {{ old('status', $borrowing->status) == 'ditolak' ? 'selected' : '' }}>Ditolak
                        </option>
                                ...
                                <div class="flex-1 w-full">
                                    ...
                                    <select name="tools[{{ $index }}][tool_id]"
                                        ...
                                        @foreach($tools as $tool)
                                            <option value="{{ $tool->id }}" data-stok="{{ $tool->stok }}" {{ old("tools.$index.tool_id", $detail->tool_id) == $tool->id ? 'selected' : '' }}>
                                                {{ $tool->nama_alat }} (Stok: {{ $tool->stok }})</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="w-full sm:w-32">
                                    <label class="block text-[10px] text-gray-400 mb-1 uppercase tracking-wider">Jumlah</label>
                                    <input type="number" name="tools[{{ $index }}][jumlah]" min="1"
                                        value="{{ old("tools.$index.jumlah", $detail->jumlah) }}"
                ...
                <div class="mb-8">
                    ...
                    <textarea name="keterangan" rows="3"
                        class="w-full bg-gray-50 dark:bg-background-dark border border-gray-300 dark:border-gray-700 text-gray-900 dark:text-white text-sm rounded-lg focus:ring-primary focus:border-primary block p-3 transition-all"
                        placeholder="Keterangan peminjaman...">{{ old('keterangan', $borrowing->keterangan) }}</textarea>
                </div>
