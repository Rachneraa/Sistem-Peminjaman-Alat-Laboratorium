...
@section('content')
    <div class="mb-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div class="flex flex-wrap gap-3 w-full md:w-auto">
            <!-- Export Button -->
            <a href="{{ route('admin.users.export') }}" data-export-link data-loading-text="Menyiapkan export users..."
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-white/5">
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-white/5">
                                ...
                                <tr class="hover:bg-gray-50 dark:hover:bg-white/5 transition-colors group">
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-4">
                                            <div
                                                class="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center overflow-hidden flex-shrink-0 group-hover:bg-primary/20 transition-colors">
                                                <span class="material-symbols-outlined text-primary text-[24px]">person</span>
                                            </div>
                                            <div class="min-w-0">
                                                <div class="text-sm font-bold text-gray-900 dark:text-white truncate">{{ $user->name }}
                                                </div>
                                                <div class="text-[10px] text-gray-500 font-mono uppercase mt-0.5">UID:
                                                    USER-{{ str_pad($user->id, 4, '0', STR_PAD_LEFT) }}</div>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-6 py-4 text-right">
                                        <div class="flex justify-end gap-2">
                                            <x-tooltip text="Edit Profile">
                                                <x-button variant="ghost" size="sm" :href="route('admin.users.edit', $user)"
                                                    class="text-primary" icon="edit" />
                                            </x-tooltip>
                                            <form method="POST" action="{{ route('admin.users.destroy', $user) }}"
                                                class="inline delete-form" data-id="{{ $user->id }}" data-name="{{ $user->name }}">
                                                @csrf
                                                @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="5">
                                <x-empty-state icon="person_off" title="Data User Kosong"
                                    description="Belum ada user yang terdaftar atau filter tidak sesuai."
                                    :actionUrl="route('admin.users.create')" actionText="Tambah User Baru" />
                            </td>
                        </tr>
                    @endforelse
                </tbody>
