...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
    ...
    <x-card class="industrial-border" :padding="false">
        <div class="p-6 flex items-center gap-4">
            ...
            <div>
                <p class="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-widest">Total Kembali</p>
    ...
    <x-card class="industrial-border" :padding="false">
        <div class="p-6 flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-2xl font-bold text-gray-900 dark:text-white mt-1">Rp{{ number_format($stats['total_fines'], 0, ',', '.') }}</p>
            </div>
        </div>
    </x-card>
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
    <x-card title="Statistik Operasional" icon="auto_graph">
        <x-slot:actions>
            <select id="periodFilter" class="bg-gray-50 dark:bg-gray-800 border-none text-xs font-bold uppercase tracking-widest text-gray-500 rounded-lg focus:ring-0 px-3 py-1 cursor-pointer">
            ...
            <div class="divide-y divide-gray-100 dark:divide-white/5">
                @foreach($popular_tools as $pop)
                    <div class="p-4 hover:bg-gray-50 dark:hover:bg-white/5 transition flex items-center gap-4 group">
                        <div class="w-12 h-12 rounded-lg bg-gray-100 dark:bg-background-dark border border-gray-200 dark:border-gray-700 overflow-hidden flex-shrink-0">
                            @if($pop->tool->gambar)
                                <img src="{{ asset($pop->tool->gambar) }}" alt="{{ $pop->tool->nama_alat }}" class="w-full h-full object-cover group-hover:scale-110 transition-transform">
                            @else
                        ...
                        <div class="flex-1 min-w-0">
                            <h4 class="font-bold text-sm text-gray-900 dark:text-white truncate group-hover:text-primary transition-colors">{{ $pop->tool->nama_alat }}</h4>
                            <p class="text-[10px] text-gray-500 uppercase tracking-widest font-mono">ID: #{{ $pop->tool->id }}</p>
                        </div>
                        <div class="text-right">
                            <div class="text-lg font-bold text-primary dark:text-accent-green font-mono">{{ $pop->total }}</div>
